...
module.exports = function (grunt) {
      ...
      options: {
        ...
        patterns: {
          ...
          js: [
            ...
            [/(locales\/.*?\.(?:json))/gm, 'Update the JS to reference our revved json']
          ],
          json: [
            // [/(images\/.*?\.(?:gif|jpeg|jpg|png|webp|svg))/gm, 'Update the JSON to reference our revved images'],
            [/(\w*?\.(?:gif|jpeg|jpg|png|webp|svg))/gm, 'Update the JSON to reference our revved images'],
          ]
        }
      }
    },
  ...
  grunt.registerTask('release', 'bump and changelog', function(type) {
    grunt.task.run([
      ...
      'bump-commit'
      // 'conventionalGithubReleaser',  // only for GitHub
    ]);
  });
